...
spec:
  ...
  quota:
    limit: 10`),
		...
		{
			desc: "quota period must be less than 1 hour",
			manifest: []byte(`
apiVersion: hub.traefik.io/v1alpha1
kind: APIPlan
metadata:
...
spec:
  ...
  quota:
    limit: 1
    period: 2h`),
			wantErrs: field.ErrorList{{Type: field.ErrorTypeInvalid, Field: "spec.quota.period", BadValue: "string", Detail: "must be between 1s and 1h"}},
		},
		{
...
spec:
  ...
  quota:
    limit: 1`),
			wantErrs: field.ErrorList{{Type: field.ErrorTypeNotSupported, Field: "spec.rateLimit.strategy", BadValue: "yolo", Detail: "supported values: \"local\", \"distributed\""}},
		},
	}
